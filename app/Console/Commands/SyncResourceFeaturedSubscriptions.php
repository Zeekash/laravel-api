...
use App\Mail\ResourceFeaturedSlotRenewedMail;
...
class SyncResourceFeaturedSubscriptions extends Command
{
    protected $signature = 'resources:sync-featured-subscriptions';
    protected $description = 'Sync resource featured ad subscriptions with Stripe, store paid invoices, send renewal/cancellation emails, and release expired canceled slots.';
    ...
    public function handle(): int
    {
        ...
        ResourceFeaturedSubscription::with(['company', 'resourcePage', 'slot'])
            ...
            ->chunkById(50, function ($subscriptions) use ($stripe) {
                foreach ($subscriptions as $subscription) {
                    try {
                        ...
                        $this->storePaidInvoices($stripe, $subscription->fresh(['company', 'resourcePage', 'slot']));
                    } catch (\Throwable $e) {
                        report($e);
                        $this->warn("Sync failed for resource featured subscription #{$subscription->id}: {$e->getMessage()}");
                    }
                }
            });
            ...
            ->chunkById(50, function ($subscriptions) use ($stripe) {
                foreach ($subscriptions as $subscription) {
                    try {
                        ...
                        $this->info("Released resource featured slot for subscription #{$subscription->id}");
                    } catch (\Throwable $e) {
                        report($e);
    ...
    private function storePaidInvoices(StripeClient $stripe, ResourceFeaturedSubscription $subscription): void
    {
        if (! $subscription->stripe_subscription_id) {
            return;
        }
    ...
    private function sendRenewalEmails(?ResourceFeaturedSubscription $subscription, ResourceFeaturedSubscriptionInvoice $invoice): void
    {
        ...
        foreach ($this->mailRecipients($subscription) as $recipient) {
            try {
                Mail::to($recipient['email'])->send(new ResourceFeaturedSlotRenewedMail($subscription, $invoice, $recipient['is_admin']));
            } catch (\Throwable $e) {
                Log::error('Resource featured renewal email failed', ['to' => $recipient['email'], 'error' => $e->getMessage()]);
            }
        }
    }
    ...
    private function sendCancellationEmails(?ResourceFeaturedSubscription $subscription): void
    {
        ...
        foreach ($this->mailRecipients($subscription) as $recipient) {
            try {
                ...
                Log::error('Resource featured cancellation email failed', ['to' => $recipient['email'], 'error' => $e->getMessage()]);
            }
        }
    }
