...
use App\Mail\ResourceFeaturedSlotCancelledMail;
use App\Mail\ResourceFeaturedSlotPurchasedMail;
...
class ResourceFeaturedAdController extends Controller
{
    public function index(Request $request)
    {
        ...
        $resources = $query->paginate(24)->appends($request->query());

        $subscriptions = ResourceFeaturedSubscription::query()
            ->with(['resourcePage', 'slot'])
            ->where('company_id', $company->id)
            ->whereIn('status', ['active', 'past_due'])
            ->latest()
            ->get();

        return view('company.resource-featured-ads.index', compact('resources', 'subscriptions', 'search', 'company'));
    }

    public function checkout(Request $request)
    {
        $request->validate([
            'slot_id' => ['required', 'integer', 'exists:resource_featured_slots,id'],
        ...
        if (empty($company->stripe_payment_method_id)) {
            return $this->redirectToPaymentMethods('Please add a card first before reserving a resource sponsored slot.');
        }
        ...
        $slot = ResourceFeaturedSlot::query()
            ->with(['resourcePage'])
            ->findOrFail($slotId);
        ...
        DB::transaction(function () use ($slotId, $company, $billingCycle, $priceCents, $stripeCustomerId, &$pending) {
            $lockedSlot = ResourceFeaturedSlot::query()->lockForUpdate()->findOrFail($slotId);

            $isTaken = ResourceFeaturedSubscription::query()
                ->where('resource_featured_slot_id', $lockedSlot->id)
                ->whereIn('status', ['active', 'past_due'])
                ->where(function ($q) {
            ...
            $isReserved = ResourceFeaturedSubscription::query()
                ->where('resource_featured_slot_id', $lockedSlot->id)
            ...
            $pending = ResourceFeaturedSubscription::create([
                'resource_featured_slot_id' => $lockedSlot->id,
                'resource_page_id' => $lockedSlot->resource_page_id,
        ...
        try {
            $pending->loadMissing(['slot.resourcePage', 'company']);
            ...
            $stripeSubscription = $stripe->subscriptions->create([
                ...
                'metadata' => [
                    'module' => 'resource_featured_ads',
                    'resource_featured_subscription_id' => (string) $pending->id,
                    'resource_page_id' => (string) $pending->resource_page_id,
            ...
            $this->sendPurchasedEmails($pending->fresh(['company', 'resourcePage', 'slot']));

            return redirect()->route('company.resource-featured-ads.index')
                ->with('success', 'Resource sponsored slot purchased successfully.');
        } catch (\Throwable $e) {
            report($e);
    ...
    public function cancelSubscription(ResourceFeaturedSubscription $subscription)
    {
        $company = $this->company();

        abort_if(! $company || (int) $subscription->company_id !== (int) $company->id, 403);

        $subscription->loadMissing(['company', 'resourcePage', 'slot']);
        ...
        try {
            if (! $subscription->stripe_subscription_id) {
                ...
                $this->sendCancellationEmails($subscription->fresh(['company', 'resourcePage', 'slot']));
            ...
            Log::error('Resource featured slot cancellation failed', [
    ...
    private function createStripeProductAndPrice(StripeClient $stripe, ResourceFeaturedSubscription $subscription): array
    {
        $subscription->loadMissing(['slot.resourcePage', 'company']);

        $resourceName = $subscription->resourcePage->title ?? 'Resource';
        $slotNumber = $subscription->slot->slot_number ?? '-';
        $interval = $subscription->billing_cycle === 'yearly' ? 'year' : 'month';

        $product = $stripe->products->create([
            'name' => trim("Featured Resource Ad - {$resourceName} - Slot {$slotNumber}"),
            'metadata' => [
                'module' => 'resource_featured_ads',
                'resource_featured_subscription_id' => (string) $subscription->id,
                'resource_page_id' => (string) $subscription->resource_page_id,
    ...
    private function storeStripeInvoice(ResourceFeaturedSubscription $subscription, $invoice, bool $sendRenewalEmail = true): ?ResourceFeaturedSubscriptionInvoice
    {
        ...
        $invoiceRow = ResourceFeaturedSubscriptionInvoice::updateOrCreate(
            ['stripe_invoice_id' => $invoice->id],
            [
                'resource_featured_subscription_id' => $subscription->id,
        ...
        if ($sendRenewalEmail && $invoiceRow->wasRecentlyCreated && ($invoice->status ?? null) === 'paid') {
            $this->sendRenewalEmails($subscription->fresh(['company', 'resourcePage', 'slot']), $invoiceRow);
        }

        return $invoiceRow;
    }

    private function sendPurchasedEmails(?ResourceFeaturedSubscription $subscription): void
    {
        if (! $subscription) {
            return;
        }
        ...
        foreach ($this->mailRecipients($subscription) as $recipient) {
            try {
                Mail::to($recipient['email'])->send(new ResourceFeaturedSlotPurchasedMail($subscription, $recipient['is_admin']));
            } catch (\Throwable $e) {
                Log::error('Resource featured purchase email failed', [
                    'to' => $recipient['email'],
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    private function sendCancellationEmails(?ResourceFeaturedSubscription $subscription): void
    {
        if (! $subscription) {
            return;
        }
        ...
        foreach ($this->mailRecipients($subscription) as $recipient) {
            try {
                Mail::to($recipient['email'])->send(new ResourceFeaturedSlotCancelledMail($subscription, $recipient['is_admin']));
            } catch (\Throwable $e) {
                Log::error('Resource featured cancellation email failed', [
                    'to' => $recipient['email'],
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    private function sendRenewalEmails(?ResourceFeaturedSubscription $subscription, ResourceFeaturedSubscriptionInvoice $invoice): void
    {
        if (! $subscription) {
            return;
        }
        ...
        foreach ($this->mailRecipients($subscription) as $recipient) {
            try {
                Mail::to($recipient['email'])->send(new \App\Mail\ResourceFeaturedSlotRenewedMail($subscription, $invoice, $recipient['is_admin']));
            } catch (\Throwable $e) {
                Log::error('Resource featured renewal email failed', [
                    'to' => $recipient['email'],
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    private function mailRecipients(ResourceFeaturedSubscription $subscription): array
    {
        $recipients = [];
