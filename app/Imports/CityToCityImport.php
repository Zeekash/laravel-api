...
class CityToCityImport implements ToCollection, WithChunkReading, WithBatchInserts
{
    protected $admin_id;
    ...
    public function collection(Collection $rows)
    {
        ...
        if (!empty($batchRecords)) {
            ...
            $existing = CityToCityRoute::whereIn('city_from_id', array_column($batchRecords, 'city_from_id'))
                ->whereIn('city_to_id', array_column($batchRecords, 'city_to_id'))
                ->get()
                ->keyBy(fn($item) => $item->city_from_id . '_' . $item->city_to_id);

            $newRecords = [];

            foreach ($batchRecords as $record) {
                $key = $record['city_from_id'] . '_' . $record['city_to_id'];
            ...
            // Insert new ones
            if (!empty($newRecords)) {
                DB::table('city_to_city_routes')->insert($newRecords);
