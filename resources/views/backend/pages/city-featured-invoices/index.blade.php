...
<div class="inv-wrap">
    <div class="inv-hero">
        <h1 class="inv-title">City Featured Ad Invoices</h1>
    ...
    <div class="inv-panel">
        <form class="inv-filter" method="GET">
            <input class="inv-input" name="search" value="{{ $search }}" placeholder="Search invoice, company, city...">
    ...
    <table class="inv-table">
        <thead><tr><th>Invoice</th><th>Company</th><th>City</th><th>Amount</th><th>Status</th><th>Period</th><th>Action</th></tr></thead>
        <tbody>
            @forelse($invoices as $invoice)
                <tr>
                    <td>{{ $invoice->stripe_invoice_id }}</td>
                    <td>{{ optional($invoice->company)->company_name ?? optional($invoice->company)->name ?? 'Company' }}</td>
                    <td>{{ optional(optional($invoice->subscription)->cityPage)->name ?? '-' }}</td>
                    ...
                    <td><a class="inv-btn" href="{{ route('admin.city-featured-invoices.show', $invoice->id) }}">View</a></td>
                </tr>
            @empty
