            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="data-tables">
                        ...
                        <form method="POST" action="{{ route('admin.products.deleteSelected') }}">
                            @csrf
                            @method('DELETE')
                            ...
                            <table id="dataTable" class="text-center">
                                <thead class="bg-light text-capitalize">
                                    <tr>
                                        ...
                                        <th>Description</th>
                                        <th>Price</th>
                                        @if ($user->can('product.edit') || $user->can('product.delete'))
                                        <th>Action</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        @if ($user->can('product.delete'))
                                        <td><input type="checkbox" name="ids[]" value="{{ $produtc->id }}"></td>
                                        @endif
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $produtc->name }}</td>
                                        <td>{{ $produtc->description }}</td>
                                        <td>$ {{ $produtc->price }}</td>
                                        @if ($user->can('product.edit'))
                                        <td style="display:flex;justify-content: center">
                                            @if ($user->can('product.edit') || $user->can('product.delete'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.products.edit', $produtc->id) }}">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            @endif
                                            {{-- <a class="btn btn-danger text-white"
                                                        onclick="return confirm('Are you sure you want to delete this?')"
                                                        href="{{ route('admin.products.destroy', $produtc->id) }}"><i
                                                class="fa fa-trash"></i></a> --}}
                                            @if ($user->can('product.delete'))
                                            <form method="POST"
                                                action="{{ route('admin.products.destroy', $produtc->id) }}">
                                                @csrf
