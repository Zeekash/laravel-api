...
<div class="cf-wrap">
    <div class="cf-hero">
        <div class="cf-hero-top">
            <div>
                <div class="cf-kicker">★ Sponsored resources management</div>
                <h1 class="cf-title">Resource Featured Ad Slots</h1>
                <p class="cf-sub">Manage monthly/yearly pricing, discounts, perks, availability, and active sponsored slots for each resource.</p>
            </div>
    ...
    <div class="cf-panel">
        <form class="cf-filter" method="GET">
            <div class="cf-field" style="min-width:260px">
                ...
                <input class="cf-input" name="search" value="{{ $search }}" placeholder="Search resource title or slug...">
            </div>

            <div class="cf-field" style="min-width:260px">
                <label>Filter Resource</label>
                <select class="cf-select" name="resource_page_id">
                    <option value="">All resources</option>
                    @foreach($resources as $resource)
                        <option value="{{ $resource->id }}" @selected($selectedResourceId == $resource->id)>
                            {{ $resource->title }}
                        </option>
                    @endforeach
                </select>
            ...
            @if(!empty($selectedResourceId) || !empty($selectedSlotStatus) || !empty($search))
                <a class="cf-btn cf-btn-light" href="{{ url()->current() }}">Clear</a>
            @endif
        </form>
    </div>

    <form method="POST" action="{{ route('admin.resource-featured-slots.updateMany') }}" id="cfSlotsForm">
        @csrf

        <div class="cf-toolbar">
            <div>
                <div class="cf-toolbar-title">Bulk slot editor</div>
                <div class="cf-toolbar-note">Edit resource slots and save all changes once.</div>
            </div>
            <button class="cf-btn" type="submit">Save Changes</button>
        </div>

        @forelse($resourcesWithSlots as $resource)
            @php
                $resourceTaken = $resource->featuredSlots->filter(fn ($slot) => $slotFinalStatus($slot) === 'taken')->count();
                $resourceReserved = $resource->featuredSlots->filter(fn ($slot) => $slotFinalStatus($slot) === 'reserved')->count();
                $resourceInactive = $resource->featuredSlots->filter(fn ($slot) => $slotFinalStatus($slot) === 'inactive')->count();
                $resourceAvailable = $resource->featuredSlots->filter(fn ($slot) => $slotFinalStatus($slot) === 'available')->count();
            @endphp

            <section class="cf-resource-card">
                <div class="cf-resource-head">
                    <div class="cf-resource-left">
                        <div class="cf-resource-avatar">{{ strtoupper(substr($resource->title, 0, 2)) }}</div>
                        <div>
                            <h2 class="cf-resource-name">{{ $resource->title }}</h2>
                            <div class="cf-resource-meta">3 sponsored slots available per resource</div>
                        </div>
                    </div>

                    <div class="cf-pills">
                        <span class="cf-pill available">{{ $resourceAvailable }} Available</span>
                        <span class="cf-pill taken">{{ $resourceTaken }} Taken</span>
                        @if($resourceReserved > 0)<span class="cf-pill reserved">{{ $resourceReserved }} Reserved</span>@endif
                        @if($resourceInactive > 0)<span class="cf-pill inactive">{{ $resourceInactive }} Inactive</span>@endif
                    </div>
                </div>

                <div class="cf-slot-grid">
                    @foreach($resource->featuredSlots as $slot)
                        @php
                            $key = $slot->id;
        ...
        @empty
            <div class="cf-empty">No resources found for the selected filter.</div>
        @endforelse

        <div class="cf-toolbar">
            <div>
                <div class="cf-toolbar-title">Ready to update resource slots?</div>
    ...
    <div style="margin-top:18px">{{ $resourcesWithSlots->links() }}</div>
</div>
