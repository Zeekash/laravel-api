...
@section('title')
Soft Deleted City To State Routes
@endsection
        ...
        <div class="row align-items-center">
            <div class="col-sm-6">
                <div class="breadcrumbs-area clearfix">
                    ...
                    <h4 class="page-title pull-left">Soft Deleted City To State Routes</h4>
                    <ul class="breadcrumbs pull-left">
                        <li><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li><a href="{{ route('admin.city-to-state-routes.index') }}">City To State Routes</a></li>
    ...
    <div class="main-content-inner">
        <div class="row">
            ...
            <div class="col-12 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title float-left">Soft Deleted City To State Routes</h4>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($city_to_state_routes as $cityToStateRoute)
                                        <tr>
                                            ...
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>Moving from {{ $cityToStateRoute->cityFrom->name }} to {{ $cityToStateRoute->stateTo->name }}</td>
                                            <td>{{ $cityToStateRoute->cityFrom->name }}</td>
                                            <td>{{ $cityToStateRoute->stateTo->name }}</td>
                                            {{-- <td>{{ $cityToStateRoute->miles }}</td> --}}
                                            {{-- <td>{{ $cityToStateRoute->min_cost }}</td> --}}
                                            {{-- <td>{{ $cityToStateRoute->max_cost }}</td> --}}
                                            <td>{{ $cityToStateRoute->deleted_at->format('Y-m-d H:i:s') }}</td>
                                            <td>{{ $cityToStateRoute->deletedBy?->name ?? 'N/A' }}</td>
                                            <td style="display:flex;justify-content: center">
                                                <form action="{{ route('admin.city-to-state.restore', $cityToStateRoute->id) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                ...
                                                <form action="{{ route('admin.city-to-state.forceDelete', $cityToStateRoute->id) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
