...
@section('title','State To State Route Edit')
@section('styles')
        ...
        <div class="col-lg-12 col-ml-12">
            <div class="row">
                ...
                <div class="col-12 mt-5">
                    <a href="{{ route('admin.state-to-state-routes.index') }}" class="btn btn-primary float-right "> Back</a>
                    <h4 class="header-title">State To State Route - Edit</h4>
                    <div class="clearfix"></div>
                    @include('flash-message')
                    <form action="{{ route('admin.state-to-state-routes.update', $stateToStateRoute->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        {{-- <div class="card mt-2">
                            <div class="card-body">
                        ...
                        <div class="card mt-4">
                            <div class="card-body">
                                ...
                                {{-- <div class="row">
                                    <div class="col-lg-4 col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="miles" value="{{ $stateToStateRoute->miles }}" class="form-control">
                                ...
                                <div class="row">
                                    <div class="col-lg-4 col-12">
                                        <div class="form-group">
                                            <label for="example-text-input" class="col-form-label">State From</label>
                                            <Select class="form-control select2" name="state_from_id">
                                                <option value="" selected disabled>--- Select State From ---</option>
                                                @foreach ($states as $stateFrom)
                                                    <option value="{{ $stateFrom->id }}" {{ $stateToStateRoute->state_from_id == $stateFrom->id ? 'selected' : '' }}> {{ $stateFrom->name }}</option>
                                                @endforeach
                                            </Select>
                                        </div>
                                    </div>
                                    <div class="col-lg-4 col-12">
                                        <div class="form-group">
                                            ...
                                            <Select class="form-control select2" name="state_to_id">
                                                <option value="" selected disabled>--- Select State To ---</option>
                                                @foreach ($states as $stateTo)
                                                    <option value="{{ $stateTo->id }}" {{ $stateToStateRoute->state_to_id == $stateTo->id ? 'selected' : '' }}> {{ $stateTo->name }}</option>
                                                @endforeach
                                            </Select>
                                        </div>
                                    </div>
