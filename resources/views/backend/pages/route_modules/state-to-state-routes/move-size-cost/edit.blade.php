...
@section('title', 'State To State Route Move Size Cost Edit')
@section('styles')
            ...
            <div class="col-lg-12 col-ml-12">
                <div class="row">
                    ...
                    <div class="col-12 mt-5">
                        ...
                        <h4 class="header-title">State To State Route Move Size Cost - Edit</h4>
                        @include('flash-message')
                        <form action="{{ route('admin.state-to-state-routes.move-size-cost.update', $moveSizeCost->id) }}" method="POST"
                            ...
                            <div class="card mt-4">
                                <div class="card-body">
                                    <div class="card-head">
                                        <h2>State To State Route Move Size Cost</h2>
                                    </div>

                                    <div class="row">
                                        <div class="col-lg-4 col-12">
                                            <div class="form-group">
                                                <label for="example-text-input" class="col-form-label">State To State
                                                    Route</label>
                                                <Select class="form-control select2" name="state_to_state_route_id">
                                                    <option value="" selected disabled>--- Select State to State Route ---</option>
                                                    @foreach ($state_to_state_routes as $stateToStateRoute)
                                                        <option value="{{ $stateToStateRoute->id }}" {{ $stateToStateRoute->id == $moveSizeCost->state_to_state_route_id ? 'selected' : '' }}>{{ $stateToStateRoute->stateFrom->name }} to {{ $stateToStateRoute->stateTo->name }}</option>
                                                    @endforeach
                                                </Select>
                                            </div>
                                        </div>
...
<script>
    $(document).ready(function() {
        $('.select2').select2({
            placeholder: 'Search for a State to State Route'
