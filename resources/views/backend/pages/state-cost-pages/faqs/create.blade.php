    ...
    <div class="main-content-inner">
        <div class="row">
            ...
            <div class="col-12 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">Create State Cost Page Faq</h4>
                        @include('backend.layouts.partials.messages')

                        <form action="{{ route('admin.state-cost-page-faqs.store') }}" method="POST">
                            @csrf
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        <tr id="initialRow">
                                            <td class="col-md-3">
                                                <select name="inputs[0][state_cost_page_id]" id="postSelect" class="form-control">
                                                    <option value="">Select State Cost Page</option>
                                                    @foreach ($state_cost_pages as $stateCostPage)
                                                        <option value="{{ $stateCostPage->id }}">{{ $stateCostPage->title }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td class="col-md-3">
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            function addRow() {
                ...
                <tr>
                    <td>
                        <input type="hidden" name="inputs[${rowCounter}][state_cost_page_id]" value="${$('#postSelect').val()}">
    ...
    <script>
        $(document).ready(function() {
            $('#postSelect').select2({
                placeholder: 'Search for a state cost page',
