...
@section('content')
@php
    $cityPayload = $cities->getCollection()->map(function ($city) {
        $slots = $city->featuredSlots;
...
<style>
    .city-ad-page{background:#f6f8fc;border-radius:24px;padding:24px;color:#111827}.city-ad-hero{background:#fff;border:1px solid #e5e7eb;border-radius:24px;padding:24px;box-shadow:0 18px 45px rgba(15,23,42,.08);margin-bottom:18px}.city-ad-kicker{font-size:13px;font-weight:800;color:#2563eb;margin-bottom:8px}.city-ad-title{font-size:30px;line-height:1.1;margin:0;font-weight:900;color:#0f172a}.city-ad-sub{color:#64748b;margin:10px 0 0}.city-ad-search{width:100%;border:1px solid #cbd5e1;border-radius:14px;padding:14px 16px;margin-top:18px;outline:none}.city-ad-search:focus{border-color:#2563eb;box-shadow:0 0 0 4px rgba(37,99,235,.12)}.city-ad-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.city-ad-card{border:1px solid #dbe3ef;background:#fff;border-radius:16px;padding:17px;text-align:left;cursor:pointer;transition:.18s ease;min-height:112px}.city-ad-card:hover{transform:translateY(-2px);box-shadow:0 16px 35px rgba(15,23,42,.08);border-color:#2563eb}.city-ad-card.full{opacity:.68}.city-name{font-size:17px;font-weight:900;color:#0f172a}.city-meta{font-size:13px;color:#475569;margin-top:6px}.city-free{font-size:13px;font-weight:900;color:#0f766e;margin-top:12px}.city-full{font-size:13px;font-weight:900;color:#991b1b;margin-top:12px}.city-sub-card{background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:16px;margin-bottom:18px}.sub-list{display:flex;gap:10px;flex-wrap:wrap}.sub-item{border:1px solid #e5e7eb;border-radius:14px;padding:12px;background:#f8fafc;min-width:250px}.sub-title{font-weight:900}.sub-meta{font-size:12px;color:#64748b;margin-top:4px}.sub-cancel{border:0;background:#ef4444;color:#fff;border-radius:10px;padding:8px 12px;font-weight:800;margin-top:10px}.modal-overlay{position:fixed;inset:0;background:rgba(15,23,42,.55);display:none;align-items:center;justify-content:center;padding:18px;z-index:9999}.modal-overlay.show{display:flex}.city-modal{width:min(680px,100%);background:#fff;border-radius:24px;box-shadow:0 30px 80px rgba(15,23,42,.35);padding:24px;max-height:92vh;overflow:auto}.modal-head{display:flex;justify-content:space-between;gap:16px}.modal-title{font-size:27px;font-weight:950;margin:0;color:#0f172a}.modal-sub{color:#64748b;margin-top:5px}.modal-close{width:42px;height:42px;border-radius:14px;border:1px solid #cbd5e1;background:#fff;font-size:24px;line-height:1}.section-label{font-size:13px;font-weight:900;color:#0f172a;margin:20px 0 10px}.slot-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.slot-option{border:1px solid #dbe3ef;border-radius:16px;padding:15px;cursor:pointer;background:#fff;min-height:120px;position:relative}.slot-option.selected{border-color:#2563eb;box-shadow:0 0 0 3px rgba(37,99,235,.12);background:#eff6ff}.slot-option.unavailable{opacity:.5;cursor:not-allowed;background:#f8fafc}.slot-label{font-size:13px;font-weight:900;color:#475569;text-transform:uppercase}.slot-status{font-size:18px;font-weight:950;margin-top:12px;color:#0f172a}.slot-price{font-size:14px;color:#475569;margin-top:6px}.slot-taken{font-size:12px;color:#64748b;margin-top:8px}.billing-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.billing-option{border:1px solid #dbe3ef;border-radius:16px;padding:16px;display:flex;align-items:center;gap:10px;cursor:pointer;font-weight:900}.billing-option.selected{border-color:#2563eb;background:#eff6ff}.perks-box{border:1px solid #bfdbfe;background:#eff6ff;border-radius:16px;padding:14px;color:#1e3a8a;margin-top:16px}.total-box{background:#f8fafc;border:1px solid #e5e7eb;border-radius:16px;padding:18px;margin-top:16px;display:flex;justify-content:space-between;align-items:center}.total-box span{color:#475569}.total-box strong{font-size:27px;color:#0f172a}.reserve-btn{width:100%;border:0;background:#2563eb;color:#fff;border-radius:16px;padding:16px;font-weight:950;margin-top:14px}.reserve-btn:disabled{opacity:.5;cursor:not-allowed}.verified-note{text-align:center;color:#64748b;font-size:13px;margin-top:12px}.alert{padding:13px 15px;border-radius:14px;margin-bottom:14px;font-weight:800}.alert-success{background:#dcfce7;color:#166534}.alert-danger{background:#fee2e2;color:#991b1b}@media(max-width:1200px){.city-ad-grid{grid-template-columns:repeat(3,1fr)}}@media(max-width:900px){.city-ad-grid,.slot-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:640px){.city-ad-page{padding:14px}.city-ad-grid,.slot-grid,.billing-grid{grid-template-columns:1fr}.city-ad-title{font-size:24px}}
</style>

<div class="city-ad-page">
    @if(session('success')) <div class="alert alert-success">{{ session('success') }}</div> @endif
    @if(session('error')) <div class="alert alert-danger">{{ session('error') }}</div> @endif

    <div class="city-ad-hero">
        <div class="city-ad-kicker">Featured ads · Cities</div>
        <h1 class="city-ad-title">Reserve sponsored city slots</h1>
        <p class="city-ad-sub">Each city has 3 sponsored slots. Available slots can be reserved monthly or yearly using your default card.</p>

        <form method="GET">
            <input class="city-ad-search" name="search" value="{{ $search }}" placeholder="Search available cities...">
        </form>
    </div>

    @if($subscriptions->count())
        <div class="city-sub-card">
            <strong>Your active city sponsored slots</strong>
            <div class="sub-list" style="margin-top:12px">
                @foreach($subscriptions as $subscription)
                    <div class="sub-item">
                        <div class="sub-title">
                            {{ $subscription->cityPage->name ?? 'City' }}@if(optional($subscription->cityPage->state)->abv), {{ optional($subscription->cityPage->state)->abv }}@endif
                        ...
                        @if(! $subscription->cancel_at_period_end)
                            <form method="POST" action="{{ route('company.city-featured-ads.cancel', $subscription->id) }}" onsubmit="return confirm('Cancel this subscription at period end?')">
                                @csrf
    ...
    <div class="city-ad-grid">
        @foreach($cities as $city)
            @php
                $free = $city->featuredSlots->filter(fn ($slot) => $slot->isAvailable())->count();
                $lowest = $city->featuredSlots->where('is_active', true)->min('monthly_price_cents');
                $stateAbv = optional($city->state)->abv ?? optional($city->state)->abbr ?? '';
            @endphp
            <button type="button" class="city-ad-card {{ $free == 0 ? 'full' : '' }}" data-city-id="{{ $city->id }}">
                ...
                @if($free > 0)
                    <div class="city-free">{{ $free }} of 3 free</div>
                @else
                    <div class="city-full">0 of 3 — full</div>
                @endif
            </button>
        @endforeach
    </div>

    <div style="margin-top:18px">{{ $cities->links() }}</div>
</div>
...
<div class="modal-overlay" id="cityModalOverlay">
    <div class="city-modal">
        <div class="modal-head">
            <div>
                <h2 class="modal-title" id="modalCityTitle">City</h2>
                <div class="modal-sub">City featured slot · “Sponsored” labeled</div>
            </div>
            <button class="modal-close" type="button" id="modalClose">×</button>
        </div>

        <form method="POST" action="{{ route('company.city-featured-ads.checkout') }}" id="reserveForm">
            @csrf
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const title = document.getElementById('modalCityTitle');
        ...
        function selectSlot(slotId) {
            selectedSlot = currentCity.slots.find(s => String(s.id) === String(slotId));
        ...
        function openCity(cityId) {
            currentCity = cities.find(c => String(c.id) === String(cityId));
            ...
            title.textContent = currentCity.name + (currentCity.state_abv ? ', ' + currentCity.state_abv : '');
            slotGrid.innerHTML = currentCity.slots.map(slot => {
            ...
            const firstAvailable = currentCity.slots.find(s => s.available);
            if (firstAvailable) {
        ...
        document.querySelectorAll('[data-city-id]').forEach(card => {
            card.addEventListener('click', () => openCity(card.dataset.cityId));
        });
