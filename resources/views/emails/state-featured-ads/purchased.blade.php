@php
    ...
    $totalCents = (int) ($subscription->total_price_cents ?: (($subscription->price_cents ?? 0) + ($subscription->lead_addon_price_cents ?? 0)));
...
<head>
    <meta charset="utf-8">
    <title>Sponsored State Slot Purchased</title>
</head>
    ...
    <tr>
        <td align="center">
            <table width="620" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
                <tr>
                    <td style="background:#0f2a4a;padding:24px 28px;color:#ffffff;">
                        <h1 style="margin:0;font-size:22px;line-height:1.35;">
                            {{ $admin ? 'New sponsored state slot purchase' : 'Your sponsored state slot is active' }}
                        </h1>
                    </td>
                </tr>
                <tr>
                    <td style="padding:28px;">
                        <p style="font-size:15px;line-height:1.7;margin:0 0 18px;">
                            @if($admin)
                                {{ $companyName }} has purchased a sponsored state slot.
                            @else
                                Thanks, {{ $companyName }}. Your sponsored state slot has been activated successfully.
                            @endif
                        </p>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
                            <tr><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;font-weight:bold;width:190px;">State</td><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">{{ $stateName }}</td></tr>
                            ...
                            <tr><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;font-weight:bold;">Billing</td><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">${{ $amount }}/{{ $cycle }}@if($subscription->lead_addon_selected) · includes {{ $subscription->lead_addon_leads_count ?: 5 }} leads add-on @endif</td></tr>
                            <tr><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;font-weight:bold;">Started</td><td style="padding:12px 16px;border-bottom:1px solid #e5e7eb;">{{ optional($subscription->starts_at)->format('M d, Y') ?? now()->format('M d, Y') }}</td></tr>
                        ...
                        @if($invoice && $invoice->hosted_invoice_url)
                            <p style="margin:22px 0 0;">
                                <a href="{{ $invoice->hosted_invoice_url }}" style="display:inline-block;background:#0f2a4a;color:#ffffff;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:bold;">View invoice</a>
                            </p>
                        @endif
